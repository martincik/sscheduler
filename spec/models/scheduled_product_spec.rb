...
describe ScheduledProduct do
  ...
  it "should create a new scheduled product" do
    lambda{ScheduledProduct.schedule(@products_ids, @from, @to, @store_id)}.should change{ScheduledProduct.find(:all).count}.by(3)
  end

  it "should update scheduled product" do
    ScheduledProduct.schedule(@products_ids, @from, @to, @store_id)
    ScheduledProduct.find(:all).should have(3).items
    from2 = 1.hours.ago
    lambda{ScheduledProduct.schedule(['11'], from2, @to, @store_id)}.should change{
